...
let selected=[],topic='Bản thân',positions=topicSpreads[topic],phase='question',flipped=[],cardInsights=[],resultStarted=false;
...
function resetRitual(){selected=[];flipped=[];cardInsights=[];resultStarted=false;phase='breathe';renderSlots();$('#spread-deck').hidden=false;$('#spread-deck').disabled=false;$('#hold-progress').textContent='';status('Hít thở thật sâu. Giữ câu hỏi trong lòng, rồi nhấn nút để trải bài.')}
...
function flipCard(index){if(phase!=='flipping'||flipped[index])return;flipped[index]=true;renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);const n=flipped.filter(Boolean).length;if(n===5){phase='reading-cards';status('Năm lá đã hiện. Đang luận từng lá…');readCardsInOrder()}else status(`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`)}
async function readCardsInOrder(){for(let index=0;index<5;index++){renderRevealedCards(index);status(`Đang luận lá ${index+1} · ${positions[index]}…`);const insight=await fetchCardInsight(index);cardInsights[index]=insight;renderRevealedCards(index)}phase='result';status('Năm lá đã hiện. Đang ghép bản đồ từ trải bài…');setTimeout(showResult,300)}
function cardRows(readingIndex){return selected.map((c,i)=>i<=readingIndex?`<article class="reading-card ${cardInsights[i]?'':'is-reading'}" data-card-index="${i}" title="Nhấn để xem câu chuyện của lá bài"><div class="reading-number">${i+1}</div><img src="${c.img}" alt="${c.name}" class="reading-img ${c.reversed?'reversed':''}"><div class="reading-text"><p class="card-position">${positions[i]}</p><strong>${c.name} <span class="orientation ${c.reversed?'reversed-label':''}">${c.reversed?'Ngược':'Xuôi'}</span></strong><p>${cardInsights[i]||'<span class="ai-loading">✦ Đang luận lá bài này…</span>'}</p></div></article>`:'').join('')}
function renderRevealedCards(readingIndex=4){const count=cardInsights.filter(Boolean).length;$('#result-topic').textContent=topic;$('#result-question').textContent=`“${$('#question').value.trim()}”`;$('#reading').innerHTML=`<section class="cards-map"><p class="reading-kicker">NĂM LÁ NÓI GÌ</p><p class="cards-intro">Mỗi lá trả lời một phần khác nhau của câu hỏi. Đọc theo thứ tự 1 đến 5. ${count}/5 lá đã được luận.</p>${cardRows(readingIndex)}</section>`;if(!resultStarted){resultStarted=true;$('#result').classList.remove('hidden');$('#result').scrollIntoView({behavior:'smooth',block:'start'})}}
...
function followUpMarkup(){return `<section class="follow-up follow-up-composer"><h3>✦ Bạn muốn hỏi thêm gì về trải bài này?</h3><textarea class="follow-up-question" maxlength="300" rows="3" placeholder="Ví dụ: Điều gì khiến mình chưa thể đưa ra quyết định?"></textarea><button class="follow-up-send" type="button">Hỏi thêm ✦</button></section>`}
function bindFollowUp(composer){composer.querySelector('.follow-up-send').onclick=()=>sendFollowUp(composer)}
function showResult(){renderRevealedCards();$('#reading').insertAdjacentHTML('beforeend','<div class="ai-section"><p class="ai-loading">✦ Đang ghép các dấu hiệu thành một bản đồ cho bạn…</p></div>'+followUpMarkup());bindFollowUp($('#reading .follow-up-composer'));fetchAIReading($('#question').value.trim())}
const AI_URL='/api/tarot';
const FOLLOW_UP_URL='/api/follow-up';
async function sendFollowUp(composer){const input=composer.querySelector('.follow-up-question'),followup=input.value.trim();if(!followup)return input.focus();const thread=document.createElement('section');thread.className='follow-up follow-up-thread';thread.innerHTML=`<h3>✦ Câu hỏi của bạn</h3><p class="follow-up-user-question"></p><h3>✦ Luận giải thêm</h3><div class="follow-up-result"><p class="ai-loading">✦ Đang đọc lại trải bài…</p></div>`;thread.querySelector('.follow-up-user-question').textContent=followup;composer.replaceWith(thread);const out=thread.querySelector('.follow-up-result');try{const csrf=await fetch('/api/csrf').then(r=>r.json());const res=await fetch(FOLLOW_UP_URL,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf.token||''},body:JSON.stringify({question:$('#question').value.trim(),followup,cards:selected.map((c,i)=>({position:positions[i],name:c.name,orientation:c.reversed?'Ngược':'Xuôi'}))})});if(!res.ok)throw new Error(`HTTP ${res.status}`);const reader=res.body.getReader(),decoder=new TextDecoder();let text='',buf='';out.textContent='';while(true){const {done,value}=await reader.read();if(done)break;buf+=decoder.decode(value,{stream:true});const lines=buf.split('\n');buf=lines.pop();for(const line of lines){const t=line.trim();if(!t.startsWith('data: ')||t==='data: [DONE]')continue;try{const d=JSON.parse(t.slice(6)).choices?.[0]?.delta?.content;if(d){text+=d;out.textContent=text}}catch{}}}}catch(err){out.textContent='Không thể đọc thêm lúc này. Hãy thử lại sau ít phút.'}finally{thread.insertAdjacentHTML('afterend',followUpMarkup());bindFollowUp(thread.nextElementSibling)}}
