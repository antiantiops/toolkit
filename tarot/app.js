...
let selected=[],topic='Bản thân',positions=topicSpreads[topic],phase='question',flipped=[],cardInsights=[];
...
function resetRitual(){selected=[];flipped=[];cardInsights=[];phase='breathe';renderSlots();$('#spread-deck').hidden=false;$('#spread-deck').disabled=false;$('#hold-progress').textContent='';status('Hít thở thật sâu. Giữ câu hỏi trong lòng, rồi nhấn nút để trải bài.')}
...
async function flipCard(index){if(phase!=='flipping'||flipped[index])return;phase='reading-card';status(`Đang đọc lá ${index+1} cho câu hỏi của bạn…`);$('#hold-progress').textContent='';const insight=await fetchCardInsight(index);if(phase!=='reading-card')return;cardInsights[index]=insight;flipped[index]=true;phase='flipping';renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);const n=flipped.filter(Boolean).length;$('#hold-progress').textContent=insight;status(`Lá ${index+1} · ${positions[index]}`);if(n===5){phase='result';setTimeout(showResult,700)}else setTimeout(()=>status(`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`),1800)}
