...
function cardRows(){return selected.map((c,i)=>`<article class="reading-card"><div class="reading-number">${i+1}</div><img src="${c.img}" alt="${c.name}" class="reading-img ${c.reversed?'reversed':''}"><div class="reading-text"><p class="card-position">${positions[i]}</p><strong>${c.name} <span class="orientation ${c.reversed?'reversed-label':''}">${c.reversed?'Ngược':'Xuôi'}</span></strong><p>${c.meaning}${c.reversed?' Khi ở chiều ngược, lá bài nhắc bạn chậm lại và nhìn phần chưa cân bằng.':''}</p></div></article>`).join('')}
...
function liveMarkup(){const cardSections=selected.map((card,i)=>`<section class="reading-section card-insight"><h3>Lá ${i+1} · ${positions[i]}</h3><p class="insight-card-name">${card.name} · ${card.reversed?'Ngược':'Xuôi'}</p><p data-ai-part="${i}"></p></section>`).join('');return `<div class="ai-result"><strong>✦ Luận giải từ trải bài</strong><p id="ai-live-text" class="ai-text"></p><div id="ai-sections" class="hidden">${cardSections}<section class="reading-section advice"><h3>Lời khuyên cho bạn</h3><p data-ai-part="5"></p></section><section class="reading-section future"><h3>Xu hướng tương lai gần</h3><p data-ai-part="6"></p></section></div></div>`}
