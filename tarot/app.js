...
let selected=[],topic='Bản thân',positions=topicSpreads[topic],phase='question',flipped=[],cardInsights=[],cardInsightsPromise=null;
...
function resetRitual(){selected=[];flipped=[];cardInsights=[];cardInsightsPromise=null;phase='breathe';renderSlots();$('#spread-deck').hidden=false;$('#spread-deck').disabled=false;$('#hold-progress').textContent='';status('Hít thở thật sâu. Giữ câu hỏi trong lòng, rồi nhấn nút để trải bài.')}
...
function completeSpread(){if(phase!=='breathe')return;phase='dealing';$('#spread-deck').hidden=true;selected=shuffle(deck.map((card,deckIdx)=>({...card,deckIdx}))).slice(0,5).map(card=>({...card,reversed:Math.random()<.32}));flipped=[];cardInsightsPromise=fetchCardInsights();status('Năm lá đang xuất hiện…');renderSlots();setTimeout(()=>{if(phase!=='dealing')return;phase='flipping';status('Năm lá đã được trải. Chạm từng lá để lật.');renderSlots()},2600)}
async function flipCard(index){if(phase!=='flipping'||flipped[index])return;if(cardInsightsPromise){status(`Đang đọc lá ${index+1} cho câu hỏi của bạn…`);await cardInsightsPromise;if(phase!=='flipping'||flipped[index])return}flipped[index]=true;renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);const n=flipped.filter(Boolean).length;status(cardInsights[index]||`Lá ${index+1} đã hiện.`);if(n===5){phase='result';setTimeout(showResult,700)}else setTimeout(()=>status(`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`),1800)}
function cardRows(){return selected.map((c,i)=>`<article class="reading-card" data-card-index="${i}" title="Nhấn để xem câu chuyện của lá bài"><div class="reading-number">${i+1}</div><img src="${c.img}" alt="${c.name}" class="reading-img ${c.reversed?'reversed':''}"><div class="reading-text"><p class="card-position">${positions[i]}</p><strong>${c.name} <span class="orientation ${c.reversed?'reversed-label':''}">${c.reversed?'Ngược':'Xuôi'}</span></strong><p>${cardInsights[i]||c.meaning}</p></div></article>`).join('')}
const CARD_INSIGHTS_URL='/api/card-insights';
async function fetchCardInsights(){try{const csrf=await fetch('/api/csrf').then(r=>r.json());const res=await fetch(CARD_INSIGHTS_URL,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf.token||''},body:JSON.stringify({question:$('#question').value.trim(),cards:selected.map((c,i)=>({position:positions[i],name:c.name,orientation:c.reversed?'Ngược':'Xuôi',keywords:c.keywords}))})});if(!res.ok)throw new Error(`HTTP ${res.status}`);const reader=res.body.getReader(),decoder=new TextDecoder();let text='',buf='';while(true){const {done,value}=await reader.read();if(done)break;buf+=decoder.decode(value,{stream:true});const lines=buf.split('\n');buf=lines.pop();for(const line of lines){const t=line.trim();if(!t.startsWith('data: ')||t==='data: [DONE]')continue;try{const d=JSON.parse(t.slice(6)).choices?.[0]?.delta?.content;if(d)text+=d}catch{}}}cardInsights=text.trim().split(/\n\s*\n/).filter(Boolean).slice(0,5)}catch(err){console.error('Card insights error:',err)}finally{cardInsightsPromise=null}}
function showResult(){$('#result-topic').textContent=topic;$('#result-question').textContent=`“${$('#question').value.trim()}”`;$('#reading').innerHTML=`<section class="cards-map"><p class="reading-kicker">NĂM LÁ NÓI GÌ</p><p class="cards-intro">Mỗi lá trả lời một phần khác nhau của câu hỏi. Đọc theo thứ tự 1 đến 5.</p>${cardRows()}</section><div class="ai-section"><p class="ai-loading">✦ Đang ghép các dấu hiệu thành một bản đồ cho bạn…</p></div>`;$('#result').classList.remove('hidden');$('#result').scrollIntoView({behavior:'smooth',block:'start'});fetchAIReading($('#question').value.trim())}
