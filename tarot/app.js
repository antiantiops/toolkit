...
async function flipCard(index){if(phase!=='flipping'||flipped[index])return;phase='reading-card';flipped[index]=true;renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);renderRevealedCards();status(`Lá ${index+1} · ${positions[index]}`);const insight=await fetchCardInsight(index);cardInsights[index]=insight;renderRevealedCards();const n=flipped.filter(Boolean).length;if(n===5){phase='result';status('Năm lá đã hiện. Đang ghép bản đồ từ trải bài…');setTimeout(showResult,300)}else{phase='flipping';status(`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`)}}
