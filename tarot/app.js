...
let selected=[],topic='Bản thân',positions=topicSpreads[topic],phase='question',flipped=[];
...
function resetRitual(){selected=[];flipped=[];phase='breathe';renderSlots();$('#spread-deck').hidden=false;$('#spread-deck').disabled=false;$('#hold-progress').textContent='';status('Hít thở thật sâu. Giữ câu hỏi trong lòng, rồi nhấn nút để trải bài.')}
...
function flipCard(index){if(phase!=='flipping'||flipped[index])return;flipped[index]=true;renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);const n=flipped.filter(Boolean).length;if(n===5){phase='result';status('Năm lá đã hiện. Đang kết nối câu chuyện của bạn…');setTimeout(showResult,700)}else status(`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`)}
function cardRows(){return selected.map((c,i)=>`<article class="reading-card" data-card-index="${i}" title="Nhấn để xem câu chuyện của lá bài"><div class="reading-number">${i+1}</div><img src="${c.img}" alt="${c.name}" class="reading-img ${c.reversed?'reversed':''}"><div class="reading-text"><p class="card-position">${positions[i]}</p><strong>${c.name} <span class="orientation ${c.reversed?'reversed-label':''}">${c.reversed?'Ngược':'Xuôi'}</span></strong><p>${c.meaning}${c.reversed?' Khi ở chiều ngược, lá bài nhắc bạn chậm lại và nhìn phần chưa cân bằng.':''}</p></div></article>`).join('')}
...
function liveMarkup(){const cardSections=selected.map((card,i)=>`<section class="reading-section card-insight"><h3>Lá ${i+1} · ${positions[i]}</h3><p class="insight-card-name">${card.name} · ${card.reversed?'Ngược':'Xuôi'}</p><p data-ai-part="${i}"></p></section>`).join('');return `<div class="ai-result"><strong>✦ Luận giải từ trải bài</strong><p id="ai-live-text" class="ai-text"></p><div id="ai-sections" class="hidden">${cardSections}<section class="reading-section advice"><h3>✦ Lời khuyên</h3><p data-ai-part="5"></p></section><section class="reading-section future"><h3>✦ Điều sắp tới</h3><p data-ai-part="6"></p></section></div></div>`}
function finishText(text,aiEl){const parts=text.trim().split(/\n\s*\n/).filter(Boolean);if(parts.length<7){aiEl.querySelector('#ai-live-text').textContent=text||'Trải bài giúp bạn nhìn rõ hơn trước bước đi tiếp.';return}aiEl.querySelector('#ai-live-text').remove();const sections=aiEl.querySelector('#ai-sections');sections.classList.remove('hidden');sections.querySelectorAll('[data-ai-part]').forEach((el,i)=>el.textContent=parts[i]||'')}
