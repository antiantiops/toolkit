...
async function flipCard(index){if(phase!=='flipping'||flipped[index])return;flipped[index]=true;renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);const n=flipped.filter(Boolean).length;$('#hold-progress').textContent='Đang đọc ý nghĩa lá bài này…';status(`Lá ${index+1} · ${positions[index]}`);const insight=await fetchCardInsight(index);cardInsights[index]=insight;$('#hold-progress').textContent=insight;if(n===5){phase='result';setTimeout(showResult,700)}else setTimeout(()=>status(`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`),1800)}
