...
let selected=[],topic='Bản thân',positions=topicSpreads[topic],phase='question',flipped=[],cardInsights=[],insightQueue=[],insightRunning=false;
...
function resetRitual(){selected=[];flipped=[];cardInsights=[];insightQueue=[];insightRunning=false;phase='breathe';renderSlots();$('#spread-deck').hidden=false;$('#spread-deck').disabled=false;$('#hold-progress').textContent='';status('Hít thở thật sâu. Giữ câu hỏi trong lòng, rồi nhấn nút để trải bài.')}
...
function flipCard(index){if(phase!=='flipping'||flipped[index])return;flipped[index]=true;renderSlots();const button=document.querySelector(`.slot-card[data-card-index="${index}"]`);button?.classList.add('just-flipped');setTimeout(()=>button?.classList.remove('just-flipped'),550);insightQueue.push(index);runInsightQueue();const n=flipped.filter(Boolean).length;status(n===5?'Năm lá đã hiện. Đang đọc từng thông điệp…':`Bạn đã lật ${n}/5 lá. Tiếp tục khi sẵn sàng.`)}
async function runInsightQueue(){if(insightRunning)return;insightRunning=true;while(insightQueue.length){const index=insightQueue.shift();const insight=await fetchCardInsight(index);cardInsights[index]=insight;if(flipped.filter(Boolean).length===5)$('#hold-progress').textContent=insight}insightRunning=false;if(flipped.filter(Boolean).length===5&&phase==='flipping'){phase='result';status('Năm lá đã hiện. Đang ghép bản đồ từ trải bài…');setTimeout(showResult,300)}}
