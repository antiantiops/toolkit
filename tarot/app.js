...
function cardRows(){return selected.map((c,i)=>cardInsights[i]?`<article class="reading-card" data-card-index="${i}" title="Nhấn để xem câu chuyện của lá bài"><div class="reading-number">${i+1}</div><img src="${c.img}" alt="${c.name}" class="reading-img ${c.reversed?'reversed':''}"><div class="reading-text"><p class="card-position">${positions[i]}</p><strong>${c.name} <span class="orientation ${c.reversed?'reversed-label':''}">${c.reversed?'Ngược':'Xuôi'}</span></strong><p>${cardInsights[i]}</p></div></article>`:'').join('')}
...
function showResult(){renderRevealedCards();$('#reading').insertAdjacentHTML('beforeend','<div class="ai-section"><p class="ai-loading">✦ Đang ghép các dấu hiệu thành một bản đồ cho bạn…</p></div>');fetchAIReading($('#question').value.trim())}
